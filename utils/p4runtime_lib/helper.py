...
class P4InfoHelper(object):
    ...
    def get(self, entity_type, name=None, id=None):
        ...
        if name:
            raise AttributeError("Could not find {} of type {}".format(name, entity_type))
        else:
            ...
            if pre.name == table_name:
                for mf in t.match_fields:
                    if name is not None:
                        ...
                        if mf.id == id:
                            return mf
        raise AttributeError("{} has no attribute {}".format(table_name, name if name is not None else id))
            ...
            if pre.name == action_name:
                for p in a.params:
                    if name is not None:
                        ...
                        if p.id == id:
                            return p
        raise AttributeError("action {} has no param {}, (has: {})".format(action_name, name if name is not None else id, a.params))
