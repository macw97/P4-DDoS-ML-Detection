...
def check_switch_conf(sw_conf, workdir):
    ...
    for conf_key in required_keys:
        if conf_key not in sw_conf or len(sw_conf[conf_key]) == 0:
            raise ConfException("missing key {} or empty value".format(conf_key))

    for conf_key in files_to_check:
        real_path = os.path.join(workdir, sw_conf[conf_key])
        if not os.path.exists(real_path):
            raise ConfException("file does not exist {}".format(real_path))
...
def program_switch(addr, device_id, sw_conf_file, workdir, proto_dump_fpath):
    ...
    try:
        ...
        error("While parsing input runtime configuration: {}".format(str(e)))
        return
    ...
    info("Connecting to P4Runtime server on {} ({})...".format(addr, target))

    if target == "bmv2":
        sw = bmv2.Bmv2SwitchConnection(address=addr, device_id=device_id,
                                       proto_dump_file=proto_dump_fpath)
    else:
        raise Exception("Don't know how to connect to target {}".format(target))
    ...
    try:
        ...
        if 'table_entries' in sw_conf:
            ...
            info("Inserting {} table entries...".format(len(table_entries)))
        ...
        if 'multicast_group_entries' in sw_conf:
            ...
            info("Inserting {} group entries...".format(len(group_entries)))
        ...
        if 'clone_session_entries' in sw_conf:
            ...
            info("Inserting {} clone entries...".format(len(clone_entries)))
