    ...
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts):
        ...
        for sw, params in switches.items():
            if "program" in params:
                ...
                # add default switch
                switchClass = None
            self.addSwitch(sw, log_file="{}/{}.log".format(log_dir, sw), cls=switchClass)
...
class ExerciseRunner:
    """
        Attributes:
            ...
            pcap : string   // directory for mininet switch pcap files
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
        ...
        """ Initializes some attributes and reads the topology json. Does not
            ...
            Arguments:
                ...
                pcap : string     // Ditto, but for mininet switch pcap files
        ...
        for dir_name in [log_dir, pcap_dir]:
            if not os.path.isdir(dir_name):
                if os.path.exists(dir_name):
                    raise Exception("{} exists and is not a directory!".format(dir_name))
                os.mkdir(dir_name)
    ...
    def program_switch_cli(self, sw_name, sw_dict):
        ...
        self.logger("Configuring switch {} with file {}".format(sw_name, cli_input_commands))
        with open(cli_input_commands, 'r') as fin:
            cli_outfile = "{}/{}_cli_output.log".format(self.log_dir, sw_name)
    ...
    def do_net_cli(self):
        ...
        print('')
        print("To view the switch output pcap, check the pcap files in {}:".format(self.pcap_dir))
        print(' for example run:  sudo tcpdump -xxx -r s1-eth1.pcap')
        print('')
        if 'grpc' in self.bmv2_exe:
            print('To view the P4Runtime requests sent to the switch, check the')
            print("corresponding txt file in {}:".format(self.log_dir))
            print(" for example run:  cat {}/s1-p4runtime-requests.txt".format(self.log_dir))
            print('')
